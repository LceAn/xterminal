...
暴露到非本机地址时，建议加 `--token` 启用访问保护：

```bash
./xterminal --host 0.0.0.0 --port 9000 --token '替换为强随机串'
```

- 页面与 `/api` 均要求令牌：可通过 `?token=...` 查询参数或 `Authorization: Bearer ...` 头提供。
- 页面首次带 `?token=` 打开后，令牌保存在浏览器 localStorage，后续自动附带。
- 校验失败一律返回 `404`（不暴露服务存在性），拒绝请求会记录日志；令牌比较使用常数时间算法。
- 未设置 `--token` 时行为与之前完全一致（默认仅 127.0.0.1）；以非回环地址监听但不设令牌时，启动日志会给出警告。
